...
def test_invalid_parameters():
    with pytest.raises(ParameterError) as exception:
        Porridge("key:secret", time_cost=1, memory_cost=1)
...
def test_str():
    porridge = Porridge(
        "key:secret,oldkey:oldsecret", memory_cost=8, time_cost=1, parallelism=1
    )
    assert (
        str(porridge)
        == "Porridge(key='key', memory_cost=8, time_cost=1, parallelism=1)"
    )
